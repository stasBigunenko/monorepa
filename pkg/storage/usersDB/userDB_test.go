...
func TestModificateUser(t *testing.T) {
	...
	us2.MapUserHTTP[id3] = user3

	t.Run("Update", func(t *testing.T) {
		user2.Name = "Glen"

		val, _ := us2.Update(context.Background(), user2)
		...
		_, err := us2.Update(context.Background(), errAccount)
		require.Errorf(t, err, "should be error not found")
	})

	t.Run("Delete=", func(t *testing.T) {
		delUser := user1
		err := us2.Delete(context.Background(), delUser.ID)
		require.Empty(t, err)
...
func TestAccountStorage_CreateAccount(t *testing.T) {
	us3 := NewUserStorage()
	val, _ := us3.Create(context.Background(), "newMan")
